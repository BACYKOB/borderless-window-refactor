...
**Решение — «grower-first, shrinker-second» + один атомарный batch (геометрия/тайминг, без рисования)**:
...
- Компенсация двойной границы: при активном флаге выставлять `BorderThickness.Left=0` из кода (`ApplyThemedBorderMetrics`), чтобы суммарно осталось 1px (XAML не трогаем — правка кодом, стиль перекрывается локальным значением; триггер Maximized→0 сохраняет поведение, т.к. в maximized inset тоже 0).
